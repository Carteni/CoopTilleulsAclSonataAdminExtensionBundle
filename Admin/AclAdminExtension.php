    ...
    public function configureQuery(
      ...
      $context = 'list'
        ...
        if (
          ...
          || $admin->isGranted(
            sprintf($admin->getSecurityHandler()->getBaseRole($admin), 'ADMIN')
          )
        ) {
            return;
        }
        ...
        foreach ($userRoles as $userRole) {
            $roles[] = ($userRole instanceof RoleInterface) ? $userRole : new Role(
              $userRole
            );
        }
        ...
        $classStmt = $this->databaseConnection->prepare(
          'SELECT id FROM acl_classes WHERE class_type = :classType'
        );
